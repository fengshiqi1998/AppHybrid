...
app.get('/api/courses/:id',(req,res)=>{
    // res.json(courses);
    res.json(scourses.filter((ele)=> ele.id == req.params.id));
});
...
const tasks = [
	new Task(1,'任务二 -- js基础练习与应用','刘冠军',
	'2018-09-30 16:35',10,13,0,77),
	new Task(2,'任务一 -- 组件交互与服务','刘冠军',
	'2018-09-21 13:54',10,7,7,76)
]
...
app.get('/api/tasks/:id',(req,res)=>{
    // res.json(courses);
    res.json(tasks.filter((ele)=> ele.id == req.params.id));
});
...
const ccourses = [
	...
	42,731),
	new Lesson(6,'Selenium IDE WEB自动化测试入门视频课程 上',
...
//     res.json(ccourses);
// })
app.get('/api/ccourses',(req,res)=>{
    res.json(ccourses);
});
app.get('/api/ccourses/:id',(req,res)=>{
    // res.json(courses);
    res.json(ccourses.filter((ele)=> ele.id == req.params.id));
});
